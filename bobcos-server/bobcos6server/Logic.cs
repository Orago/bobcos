...
namespace bobcos6server {
	...
	class Logic {
		...
		public static void AddXp(string username, int HowMuchXp, int clientid) {
			...
			if (account.level >= 100) account.level = 100;
		...
		static public int TryToCreateAccount(string username, string password, string email)
		{
			...
			if (File.Exists("accounts/" + username.ToUpper() + ".json")) return 2;
			else {
				if (password.Length < 3 || username.Length < 3) return 3;
				else {
		...
		static public bool CheckItemInInventory(string username, int itemid, int itemcount) {
			...
			foreach (InventoryTile c in i.inventory) {
				if (c.id == itemid) {
					if (c.count >= itemcount) return true;
				}
			}
		...
		static public bool AddItemToInventory(string username, int itemid, int itemcount) {
			...
			foreach (InventoryTile c in i.inventory) {
				if (c.id == itemid)
				{
					...
					//dont take item
					if (c.count > 799 || c.count + itemcount > 799) return false;
		...
		static public int CheckWorldFile(string worldname) {
			if (File.Exists($"worlds/{worldname.ToUpper()}.json")) return 1;
			else return 0;
		}
		...
		static public void JoinToWorld(object data)
		{
			...
			if (Server.Clients[idofplayer].user.World != null) return;
			...
			if (acc.StaffLevel > 2) canPass = true;
