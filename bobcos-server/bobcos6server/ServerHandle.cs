...
namespace bobcos6server {
	class ServerHandle {
		public static void WelcomeReceived(int _fromclient, Packet _packet){
			...
			if (i == "S" && Server.Clients[_fromclient].user.ServerHelloSendedToClient)
		...
		public static void HandleAccountandworld(int _Fromcliemt, Packet _packet) {
			...
			switch (selection)
			{
				case 1:
					...
					if (b == 0) ServerSend.SendWarning(_Fromcliemt, "<color=red>Wrong password!</color>");
					...
					else if (b == 2) ServerSend.SendWarning(_Fromcliemt, "<color=yellow>This account doesnt exist, create it now!</color>");
					else if (b == 3) ServerSend.SendWarning(_Fromcliemt, "<color=red>This account is already online! kicked player.</color>");
				...
				case 2:
					...
					if (c == 1) ServerSend.SendWarning(_Fromcliemt, "<color=green>Account created!</color>");
					else if (c == 2) ServerSend.SendWarning(_Fromcliemt, "<color=red>This account already exists.!</color>");
					else if (c == 3) ServerSend.SendWarning(_Fromcliemt, "<color=red>Password or username cant be shorter than 3 chartacters!</color>");
				...
				case 3:
					...
					foreach (char cha in worldname) {
						if (!AllowedCharacters3.Contains(cha)) {
							...
							if (cha == 'i') newworldname = newworldname + "I";
							else newworldname = newworldname + cha;
						}
					}
		...
		public static void FisherBobAmountDataReceived(int _fromClient, Packet _packet) {
			...
			if (itemdata.items[FishId].itemtype != "FISH") return;
			...
			else ServerSend.SendWarning(_fromClient, "<color=red>Error: You dont have enough item</color>");
		}
		...
		public static void ItemDropRequest(int _Fromcliemt, Packet _packet) {
			...
			if (!itemdata.items[Server.Clients[_Fromcliemt].user.CurrentSelecteditem].istradable) return;

			if (!Logic.CheckItemInInventory(Server.Clients[_Fromcliemt].user.username.ToUpper(), Server.Clients[_Fromcliemt].user.CurrentSelecteditem, itemcount)) return;
		...
		public static void CraftReq(int _Fromcliemt, Packet _packet) {
			...
			if (Server.Clients[_Fromcliemt].user.worldCraftDataOrder == -1) {
				...
				else CraftingSystem.BeginCrafting(_Fromcliemt, item1, item2);
			}
			else {
				...
				else CraftingSystem.BeginCrafting(_Fromcliemt, item1, item2, Server.Clients[_Fromcliemt].user.worldCraftDataOrder);
			}
		}
		...
		public static void StringFromClient(int _Fromcliemt, Packet _packet) {
			...
			if (message == "UI_SHOW_CRAFT_REQ") {
				...
				else CraftingSystem.TryToGetCraftedItem(_Fromcliemt);
			}
		...
		public static void BlockEditFromClient(int _Fromcliemt, Packet _packet) {
			...
			if (!Server.Clients[_Fromcliemt].user.CanBreakBlock) return;

			short sira = _packet.ReadShort();
			//Check world name
			if (Server.Clients[_Fromcliemt].user.World == null) return;
			...
			//rod detected
			if (i12.handitem == 83 && Server.Clients[_Fromcliemt].user.iscatchedfish) {
			...
			if (worldname != "") {
				if (itemdata.items[Server.Clients[_Fromcliemt].user.CurrentSelecteditem].itemtype == "WRENCH") {
					if (Logic.ReadWorldFg(worldname)[sira] == 0) {
						...
						if (blockid == 47) {
							...
							else ServerSend.SendChat(_Fromcliemt, "You can only watch 3 ads per 12 hours.");
						}

						if (blockid == 107) ServerSend.SendString(_Fromcliemt, "FBU");
					}
				}
				...
				if (itemdata.items[Server.Clients[_Fromcliemt].user.CurrentSelecteditem].itemtype == "FIST") {
					...
					if (accountOfUser.handitem == 31) pickaxehelpercount = 1;// pickaxe

					if (accountOfUser.handitem == 74) pickaxehelpercount = 7;// ketaru
					...
					if (Logic.ReadWorldFg(Server.Clients[_Fromcliemt].user.World.ToUpper())[sira] == 0) {
						...
						if (Logic.ReadWorldBg(Server.Clients[_Fromcliemt].user.World.ToUpper())[sira] != 0) {
							...
							if (pickaxehelpercount == 7 || Logic.worlds[Server.Clients[_Fromcliemt].user.World.ToUpper()].HealthOfBlocks[sira] == itemdata.items[Logic.ReadWorldBg(Server.Clients[_Fromcliemt].user.World.ToUpper())[sira]].blockhealth - pickaxehelpercount){
								...
								//Give gem to player
								//give xp to player
								if (pickaxehelpercount == 7) pickaxehelpercount = 0;
					...
					if (Logic.ReadWorldFg(Server.Clients[_Fromcliemt].user.World.ToUpper())[sira] != 0) return;
					...
					//Todo: if selected block is not 0, cancel block putting.
					if (Logic.ReadWorldBg(Server.Clients[_Fromcliemt].user.World.ToUpper())[sira] != 0) return;
					...
					if (Logic.ReadWorldFg(worldname)[sira] == 0) {
						...
						if (blockid == 30) {
							...
							else ServerSend.SendString(_Fromcliemt, "UI_SHOW_CRAFT");
						}
		...
		public static void TileSelected(int _Fromcliemt, Packet _packet) {
			...
			if (Server.Clients[_Fromcliemt].user.CurrentSelecteditem == (short)inventory[selecteditemsirasi].id) {
				//Show item info 
				if (itemdata.items[Server.Clients[_Fromcliemt].user.CurrentSelecteditem] == null) return;
		...
		public static void TrashRequest(int _fromclient, Packet _packet) {
			...
			if (!itemdata.items[Server.Clients[_fromclient].user.CurrentSelecteditem].istrashable) return;
